...
class TestStudent(unittest.TestCase):
    ...
    def test_do_homework(self):
        ...
        self.assertRaises(DeadlineError, self.student_example.do_homework, self.homework_inactive, "another solution")
...
class TestTeacher(unittest.TestCase):
    ...
    result_active_homework = student_example.do_homework(homework_active, 'I have done this hw')
    another_result_active_homework = student_example.do_homework(homework_active, 'I have this homework')

    result_inactive_homework = student_example.do_homework(homework_inactive, 'Done')
    another_result_inactive_homework = student_example.do_homework(homework_inactive, 'I done too')

    def test_teacher_create_homework(self):
        assert isinstance(self.teacher_example.create_homework('Active homework', 10), Homework)
        assert isinstance(self.teacher_example.create_homework('Inactive homework', 0), Homework)

    def test_check_homework(self):
        self.assertEqual(self.teacher_example.check_homework(self.result_active_homework), True)
        self.assertEqual(self.teacher_example.check_homework(self.another_result_active_homework), True)
        self.assertEqual(self.teacher_example.check_homework(self.another_result_inactive_homework), True)
        self.assertEqual(self.teacher_example.check_homework(self.result_inactive_homework), False)
        assert self.teacher_example.homework_done[self.homework_active] == ['I have done this hw', 'I have this homework']
        assert self.teacher_example.homework_done[self.homework_inactive] == ['I done too']
